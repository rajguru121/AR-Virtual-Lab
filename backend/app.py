...
def create_app() -> Flask:
    ...
    def check_dashboard_auth():
        ...
        path = request.path
        if path == '/dashboard' or path == '/frontend/templates/dashboard.html':
            token = request.cookies.get('virtulab_token')
            if not token:
                return redirect('/frontend/templates/login.html')
            from backend.services.auth_service import decode_token
            payload = decode_token(token)
            if not payload:
                resp = make_response(redirect('/frontend/templates/login.html'))
                resp.set_cookie('virtulab_token', '', expires=0)
                return resp
    ...
    def add_header(response):
        """Disable caching for all responses to force reload of static pages/scripts."""
