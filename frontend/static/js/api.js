...
const VirtuLabAPI = (() => {
    ...
    /** Register a new user */
    async function signup(name, rollNumber, email, password) {
        const data = await _request(ENDPOINTS.SIGNUP, {
            method: 'POST',
            body: { name, roll_number: rollNumber, email, password },
        });
        return data;
    }
    ...
    async function getDashboardStats() {
        return _request(ENDPOINTS.DASHBOARD_STATS);
    }

    /** Toggle the saved status of an experiment for the current user */
    async function toggleSaveExperiment(experimentId) {
        return _request(ENDPOINTS.TOGGLE_SAVE, {
            method: 'POST',
            body: {
                experiment_id: experimentId,
            },
